...
/**
 * This class contains verification of all the favourites marked by the user on
 * HomePage, Quick modal and on link portlet
 *
 * @author Shubham Raj
 * @version v1.0
 */
...
public class Favourite_TestCase extends BaseClass {
	 ...
	 *         favourite should be filtered accordingly @ exception Description of
	 *         exception you are handling @ throws Description of exception that the
	 *         method throws
	 */
	 ...
	 *         irrespective of languages selected @ throws Description of exception
	 *         that the method throws
	 */
	 ...
	 *         by default folder named 'Test Folder'
	 * 
	 */
	@Test(priority = 5)
	public static void verifyToCreateFolder() throws Throwable {
		String strTCID = "TC05_Favourite";
		LogFactory.beginTestCase(strTCID + " : Testcase to verify create Favorite folder ");
	...
	public static void verifyToDeleteFavouriteFolder() throws Throwable {
		...
		LogFactory.beginTestCase(strTCID + " : Testcase to verify delete Favorite folder ");
	...
	public static void verifyToMovingFavouriteFolderandLink() throws Throwable {
		String strTCID = "TC07_Favourite";
		LogFactory.beginTestCase(strTCID + " : verify Moving Favourite Folder and Link ");
		favouritesFactory.verifyMovingFavouriteFolderandLink(strTCID);
	}
	...
	public static void verifyToExpandAndCollapseFavroiteFolder() throws Throwable {
		String strTCID = "TC08_Favourite";
		LogFactory.beginTestCase(strTCID + " : verify Expand And Collapse Favroite Folder ");
		favouritesFactory.verifyExpandAndCollapseFavroiteFolder(strTCID);
	}
	...
	public static void VerifyToQuickModalonHomeDepartmentandalllevelsofIndexpages() throws Throwable {
		String strTCID = "TC09_Favourite";
		LogFactory.beginTestCase(
				strTCID + " : Verify Quick Favorite modal window on Homepage, Department pages and Index page");

		if (BaseClass.markedLinks.size() > 0) {

			favouritesFactory.verifyQuickModalWindowOnHomepage(strTCID);
		} else {
			ReportFactory.reporterOutput(strTCID,
					"Verify Quick Favorite modal window on Homepage, Department pages and Index page", "NA",
	...
	public static void verifySearchFilteringOnHomepage() throws Throwable {
		...
		if (BaseClass.markedLinks.size() > 0) {
			...
			LogFactory.beginTestCase(strTCID + " : verify search Filtering on favorites portlet on Homepage");
			ValidTestData = linkName;
			favouritesFactory.VerifyFilterforInvalidAndValidData(strTCID, ValidTestData);
			inValidTestData = ValidationFactory.reverseString(linkName);
			favouritesFactory.VerifyFilterforInvalidAndValidData(strTCID, inValidTestData);

		}

		else {
			ReportFactory.reporterOutput(strTCID, "Verify filtering of the favorite portlet", "NA",
					"Favorite filter should work properly with no error message",
	...
	public static void verifyToRemoveFavouriteLinkfromlinkportlet() throws Throwable {
		...
		if (BaseClass.markedLinks.size() > 0) {
			...
			LogFactory.beginTestCase(strTCID + " : Verify removing a favorite from links portlet on Department Page ");

			favouritesFactory.verifyRemoveFavouriteLink(strTCID);
		} else {
			ReportFactory.reporterOutput(strTCID, "verify Remove Favourite Link from Link portlet Department", "NA",
					"Favourite should get removed and should no more display on 'Favourite' Portlet on homepage",
					"No sufficient favourite links found to execute this test case", "PASS");
		}
	}
	...
	public static void verifyToRemoveFavouriteLinkfromQuickmodalwindow() throws Throwable {
		...
		if (BaseClass.markedLinks.size() > 0) {
			LogFactory.beginTestCase(strTCID + " : Verify removing a favorite from Quick Modal window " + linkName);
			favouritesFactory.ToRemoveFavouriteLinkfromQuickmodalwindow(strTCID);
		} else {
			ReportFactory.reporterOutput(strTCID, "verify Remove Favourite Link from Quick Modal window", "NA",
	...
	public static void verifyToRemoveFavLinkfromFavoritesPortlethomepage() throws Throwable {
		...
		LogFactory.beginTestCase(strTCID + " : Verify removing a favorite from 'Favorites Portlet' on homepage,");
		if (BaseClass.markedLinks.size() > 0) {
			favouritesFactory.ToRemoveFavLinkfromFavoritesPortlethomepage(strTCID);
		} else {
			ReportFactory.reporterOutput(strTCID, "Verify remove favourite Link from 'Favorites Portlet' on homepage",
					"NA",
	...
	public static void verifyToFavouritesOnchangingCountryGroupingType() throws Throwable {
		...
		LogFactory.beginTestCase(TCID + " : Verify favourite on changing the Country Type ");

		try {
			List<LinkedHashMap> userWCMContent = ExcelFactory
					.getUserWcmDetailsAfterFilteringCountryAndProduct("AT-Link");
			GenericFactory.navigateToHomePage();

			for (int i = 0; i < userWCMContent.size(); i++) {
				// WCMcountryGrouplist = (String)
				...
				favouritesFactory.verifyFavouritesOnchangingCountryGroupingType(TCID, eachWcmCountryGroupName, Title,
						departmentName);
